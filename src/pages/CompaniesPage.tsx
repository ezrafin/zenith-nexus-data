...
export default function CompaniesPage() {
  ...
  const filteredOrganizations = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter(org => org.name.toLowerCase().includes(query) || org.description.toLowerCase().includes(query) || org.headquarters.toLowerCase().includes(query));
    }
  ...
  return <Layout>
      <section className="section-spacing">
        <div className="container-wide">
          ...
          {/* Stats */}
          <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          delay: 0.2
            ...
            <div className="relative max-w-md">
              ...
              <Input type="text" placeholder="Search by name, description, or location..." value={searchQuery} onChange={e => {
            ...
            <div className="flex flex-wrap gap-2">
              {organizationTypes.map(type => <Button key={type.value} variant={selectedType === type.value ? 'default' : 'outline'} size="sm" onClick={() => handleTypeChange(type.value)} className="text-sm">
                  {type.label}
                </Button>)}
            </div>
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
            {paginatedOrganizations.map((org, index) => <OrganizationCard key={org.id} organization={org} index={index} />)}
          </div>

          {filteredOrganizations.length === 0 && <div className="text-center py-12 text-muted-foreground">
              No organizations found for this filter.
            </div>}

          {filteredOrganizations.length > 0 && <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />}
        </div>
      </section>
    </Layout>;
}