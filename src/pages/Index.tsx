...
export default function Index() {
  ...
  useEffect(() => {
    async function loadContent() {
      const [newsData, analyticsData] = await Promise.all([fetchNews(), fetchAnalytics()]);
      setNews(newsData);
  ...
  return <Layout>
      ...
      <section className="section-spacing-sm section-gradient">
        <div className="container-wide">
          ...
          {marketLoading && indices.length === 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {Array.from({
            length: 5
          }).map((_, i) => <SkeletonCard key={i} lines={6} />)}
            </div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
              {marketBlocks.map(block => <MarketMiniTable key={block.href} title={block.title} data={block.data} href={block.href} icon={block.icon} />)}
            </div>}
        </div>
      </section>
      ...
      <section className="section-spacing">
        <div className="container-wide">
          ...
          {contentLoading ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {Array.from({
            length: 3
          }).map((_, i) => <SkeletonCard key={i} lines={3} />)}
            </div> : <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {news.slice(0, 3).map((item, index) => <NewsCard key={item.id} news={item} featured={true} index={index} />)}
            </div>}
        </div>
      </section>
      ...
      <section className="section-spacing bg-secondary/30">
        <div className="container-wide">
          ...
          {contentLoading ? <div className="grid md:grid-cols-2 gap-6">
              {Array.from({
            length: 4
          }).map((_, i) => <SkeletonCard key={i} lines={4} />)}
            </div> : <div className="grid md:grid-cols-2 gap-6">
              {analytics.slice(0, 4).map((article, index) => <AnalyticsCard key={article.slug} article={article} index={index} />)}
            </div>}
        </div>
      </section>

      {/* Recommended Content & Market Overview */}
      {user && <section className="section-spacing bg-secondary/30">
          <div className="container-wide">
